# Auditoria histórica — 7 de setembro de 2026

- **Data da análise:** 7 de setembro de 2026.
- **Base examinada:** commit [`f6506e9`](https://github.com/forge-z/pte100/commit/f6506e9), com as alterações locais descritas no registro abaixo. O HEAD remoto não foi confirmado naquela análise.
- **Natureza:** evidência histórica, preservada integralmente a partir de `PLANO-DE-MELHORIA.md` em [`10cfb7a`](https://github.com/forge-z/pte100/commit/10cfb7aa580237e1281d8c2d64022d1951ae7162). Não é uma lista atual de defeitos nem uma declaração de que todos os critérios de aceite foram cumpridos.
- **Estado atual:** consulte o [README](../../README.md), o [ROADMAP](../../ROADMAP.md) e o [CHANGELOG](../../CHANGELOG.md).

## Correções e limites posteriores

O commit [`10cfb7a`](https://github.com/forge-z/pte100/commit/10cfb7aa580237e1281d8c2d64022d1951ae7162) corrigiu caminhos inexistentes, opções de CLI, posições UTF-8, segmentação após títulos e ordem de expansão de siglas. No revisor, acrescentou controles de Host/Origin e sessão, uma análise simultânea e tratamento de limpeza, troca de documento e respostas tardias no cliente. Também integrou as suítes do revisor a `rake verify` e excluiu dependências e saídas de build da validação editorial.

A Fase 1 de manutenção, registrada no [PR #4](https://github.com/forge-z/pte100/pull/4), restaurou a CI de verificação com dependências reproduzíveis, regeneração dos artefatos, checagem do exemplo canônico e build do site. Seus commits também atualizaram onboarding, formulários e orientações para agentes e mantenedores.

Essas entregas não encerram toda a auditoria: a validação de schemas verifica sintaxe, não instâncias; os testes do servidor chamam o dispatcher com objetos simulados, sem iniciar HTTP ou navegador; não há evidência nesses testes de fluxo completo, acessibilidade, paridade de todos os adaptadores ou precisão linguística. Pilotos, anotação humana, medições de desempenho e distribuição como pacote permanecem pendentes. Questões normativas continuam sujeitas ao processo de contribuição e governança.

A frase histórica “correções P0/P1 implementadas” deve ser lida com esses limites. O corpo a seguir conserva o texto e as evidências disponíveis à época, inclusive suas notas posteriores, sem convertê-los em validação atual.
